document.addEventListener("DOMContentLoaded", (e) => {
    async function verificarUsuario() {
        const regex = /^[VEJPG]-?\d{7,9}$/i;
        Swal.fire({
            icon: "question",
            title: "Ingrese el numero de cédula perteneciente a su usuario",
            input: "text",
            inputAttributes: {
                autocapitalize: "off",
            },
            ...
            inputValidator: (value) => {
                if (!value) {
                    return "Debe ingresar un numero de cédula para continuar.";
                } else if (!regex.test(value)) {
                    return `Cédula inválido: ${value}`;
                }
            },
            ...
            if (result.isConfirmed) {
                ...
                if (dataDB.bool) {
                    if (Object.keys(dataDB.data).length > 0) {
                        ...
                        error(
                            "Si datos",
                            "No hay datos de este usuario en el sistema SIMA.",
                        );
                    }
                } else {
                    error(
                        "Usuario no encontrado",
                        "La cédula del usuario ingresada no se encuentra registrado en el sistema SIMA.",
                    );
                }
            }
        });
        ...
        if (respuesta) {
            if (dataDB.data.respuesta_secreta === respuesta) {
                ...
                error(
                    ...
                    "La respuesta ingresada no coincide con la respuesta secreta registrada en el sistema SIMA.",
                );
            }
        }
    }
    ...
    document.getElementById("password_olvidado").addEventListener("click", verificarUsuario);
});
